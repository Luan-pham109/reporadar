...
npm run dev:publish  # duyệt draft local; nút Publish sẽ build + deploy production
...
4. Luan duyệt → đổi `draft: false`. Chạy `npm run build` để chắc schema hợp lệ, hoặc dùng `npm run dev:publish` để nút Publish tự build + deploy production.
...
Trong `/review`, bấm `Publish` sẽ đổi `draft: false`, chạy production build, rồi deploy `dist/` lên Cloudflare Pages bằng Wrangler. Nếu muốn deploy thủ công không qua review UI:
...
Membership chạy bằng Cloudflare Pages Functions:

- KV binding bắt buộc: `AUTH_KV` để lưu user, password hash và session.
- Secrets cho Google login: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`.
- Google OAuth redirect URI: `https://<domain>/api/auth/google/callback`.
...
Email/password endpoints: `/api/auth/signup`, `/api/auth/login`, `/api/auth/logout`, `/api/auth/me`.
Google login bắt đầu ở `/api/auth/google/start`.

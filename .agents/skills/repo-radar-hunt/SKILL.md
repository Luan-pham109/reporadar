...
```
Lấy từ `.github`: `stars`, `forks`, `starsPerDay`, `ageDays`, `pushedAt`, `archived`, `openIssues`, `suggestedHealthScore`; thread HN, post Reddit, và `maturitySuggestion`.

Điền vào record:
- `repoStats`: `stars`, `forks`, `note` (vd "~X ngày tuổi, ~Y sao/ngày"), `starsPerDay`, `ageDays`, `pushedAt`, `archived`, `openIssues`.
- `signalSources`: link HN/Reddit/PH tìm được.
- `scoreBreakdown.projectHealth`: lấy thẳng từ `.github.suggestedHealthScore` (0–25). Đây là thành phần duy nhất Hunt ghi vào scoreBreakdown.
...
- `practitionerGuide`: để cấu trúc rỗng/placeholder; synthesize + edit sẽ viết sau khi đã hiểu cách dùng thực tế.
